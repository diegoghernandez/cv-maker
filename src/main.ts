   ...
   async onload() {
      ...
      this.addRibbonIcon("newspaper", "Create a new cv", async () => {
         ...
         if (!(vault.getAbstractFileByPath("cvs") instanceof TFolder)) {
            new Notice(
               "cvs directory doesn't exist, It will be created in a moment",
               NOTICE_DURATION,
            );
         ...
         if (!vault.getFileByPath(BASE_CV_PATH)) {
            ...
            new BaseCVModal(this.app, (resume) => {
               ...
               extractAndFormat({ resume, settings: this.settings }).then(
                  async ([_, result]) => {
                     await vault.create(BASE_CV_PATH, result);
                     new Notice(
                        "CV information extracted, located at " +
                           BASE_CV_PATH.replace(".md", ""),
                     );
                  },
               );
            }).open();
         ...
         if (vault.getFileByPath(BASE_CV_PATH)) {
            new JobDescriptionModal(
               this.app,
               async ({ companyName, jobDescription }) => {
                  new Notice(
                     "Resume and job description send to process for llm...",
                     NOTICE_DURATION,
                  );
